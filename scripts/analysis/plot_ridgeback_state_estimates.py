...
def main():
    ...
    plt.plot(tbs[:-1], vb_nums[:, 2], label="v_theta_vicon")
    plt.plot(tb_ests, vb_ests[:, 0], label="vx_est")
    plt.plot(tb_ests, vb_ests[:, 1], label="vy_est")
    plt.plot(tb_ests, vb_ests[:, 2], label="omega_est")
